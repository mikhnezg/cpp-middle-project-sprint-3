...
TEST(Filters, AllOfCombinesPredicates) {
    ...
    db.EmplaceBack("B", "X", 1990, Genre::Fiction, 3.0, 1); // low rating
    db.EmplaceBack("C", "X", 1850, Genre::Fiction, 4.9, 1); // wrong year

    auto pred = all_of(YearBetween(1900, 1999), RatingAbove(4.5));
    auto result = filterBooks(db.begin(), db.end(), pred);
    ASSERT_EQ(result.size(), 1u);
...
TEST(Filters, AnyOfCombinesPredicates) {
    ...
    auto pred = any_of(GenreIs(Genre::SciFi), GenreIs(Genre::Biography));
    auto result = filterBooks(db.begin(), db.end(), pred);
...
// ============================================================
// string_view stability test (critical: unordered_set rehash would break this)
// ============================================================

TEST(BookDatabase, StringViewRemainsValidAfterManyInserts) {
    BookDatabase<> db;
    // Add enough books to force multiple rehashes if using unordered_set
    for (int i = 0; i < 200; ++i) {
        db.EmplaceBack("Book" + std::to_string(i), "Author" + std::to_string(i % 10),
                       2000 + i, Genre::Fiction, 4.0, i);
    }
    // Verify all string_views still point to valid strings stored in authors_
...
// ============================================================
// Statistics — additional STL functions
// ============================================================
...
TEST(Statistics, AllBooksMatch) {
    BookDatabase<> db;
    db.EmplaceBack("A", "X", 2000, Genre::Fiction, 4.0, 1);
    db.EmplaceBack("B", "Y", 2000, Genre::Fiction, 4.5, 2);

    EXPECT_TRUE(allBooksMatch(db, [](const Book &b) { return b.rating >= 3.0; }));
    EXPECT_FALSE(allBooksMatch(db, [](const Book &b) { return b.rating >= 4.3; }));
}

TEST(Statistics, ExtractRatings) {
    BookDatabase<> db;
    db.EmplaceBack("A", "X", 2000, Genre::Fiction, 3.0, 1);
    db.EmplaceBack("B", "Y", 2000, Genre::Fiction, 5.0, 2);

    auto ratings = extractRatings(db);
    ASSERT_EQ(ratings.size(), 2u);
    EXPECT_DOUBLE_EQ(ratings[0], 3.0);
    EXPECT_DOUBLE_EQ(ratings[1], 5.0);
}

// ============================================================
// Performance / stability test with large data
// ============================================================
...
TEST(Performance, LargeDatabaseInsertAndFilter) {
    ...
    const int N = 10000;
    for (int i = 0; i < N; ++i) {
        db.EmplaceBack("Book" + std::to_string(i),
                       "Author" + std::to_string(i % 100),
                       1900 + (i % 124),
                       (i % 2 == 0) ? Genre::Fiction : Genre::SciFi,
                       3.0 + (i % 20) * 0.1,
                       i);
    }
...
TEST(Performance, HistogramFlatVsRegular) {
    BookDatabase<> db;
    for (int i = 0; i < 5000; ++i) {
        db.EmplaceBack("Book" + std::to_string(i),
                       "Author" + std::to_string(i % 50),
                       2000, Genre::Fiction, 4.0, 1);
    }
    auto histFlat = buildAuthorHistogramFlat(db);
    auto histMap  = buildAuthorHistogram(db);

    EXPECT_EQ(histFlat.size(), histMap.size());
    for (const auto &[author, count] : histMap) {
        EXPECT_EQ(histFlat.at(author), count);
    }
}
